...
//*******METHODS*********
//death methods
function healthDie(containingArray) {
  return function() {
    if (this.health < 1) {
...
//firing methods
function fireMouseDown(shotType, shotArray) {
  return function() {
...
function fireAlways(shotType, shotArray) {
  return function() {
    if (true) {
      shotArray.push(new shotType(this));
    }
...
CircleShot.prototype.isAlive = function() {
  distanceDie(this);
  timeDie(this);
};
...
CircleShot.prototype.track = function() {
  trackStopNearObject(this, this.trackTarget, 5);
};
...
/*if (this.x + this.dx < instances[j][i].x + /*like this*/
/*(instances[j][i].width/2) + this.radius && this.x + this.dx > instances[j][i].x - (instances[j][i].width/2) - this.radius && this.whose !== instances[j][i] ) {
